...
- **Billing model per product** — resolved as principle (DECISIONS.md 2026-05-16): billing model is product-type-driven; no speculative billing tables. Concrete billing table (`subscriptions` / `purchases` / `credits`) is created only when the first product of that model is selected. Not a blocker now (no product selected).
