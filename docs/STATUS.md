...
- **`subscriptions` table ownership** (Волна 2, immediate) — bot-core `transport`/`payments` reference it generically but it has NO shared migration (was only in deleted food-agent migration). Decide: add shared `subscriptions` migration in `packages/database/migrations/` (likely — billing is shared infra) vs make it product-specific. Blocks bot-core subscription middleware until resolved.
