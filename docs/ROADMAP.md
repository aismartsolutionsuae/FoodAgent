...
- [x] **3.5 Smoke harness**: one stub check per layer — engine ping, support triage on fixture, experiments getFlag, qa judge, marketing dry-run (5 bot-core layers) **+ Layer-4 autonomous coder** (claude-code-action: a stub `@claude` trigger on a throwaway issue opens a no-op PR, proving the autonomous path works). Runnable locally / in CI. Layer-4 autonomous coder is tested here, NOT deferred to first product. — *done 2026-05-16: PR-A; dual-state smoke (`pnpm smoke`, vitest.smoke.config — excluded from per-PR `pnpm test`), 6 checks (ai-engine/support/qa/marketing/experiments/marketing-email) skipIf-gated, lazy per-test imports (DB client throws on empty SUPABASE_URL at load); `smoke.yml` + `layer4-smoke.yml` workflow_dispatch only (cost-discipline). Layer-4 evidence: skip — ANTHROPIC_API_KEY not provisioned; one-shot dispatch pending post-merge (workflows dispatchable only from `main`).*
- [x] **3.6 Tests**: unit tests for each layer's pure logic + `resolveUser` race. — *done 2026-05-17: PR-B; new engine.test (getPrompt map/defaults/cache, ask substitution, judge JSON), experiments.test (getFlag/isEnabled), identity.test (resolveUser existing/create/race). AI resilience (DECISIONS 2026-05-15) decided-but-unimplemented — explicitly out of scope, tracked separately (spec §3.6). Full bot-core suite 27/27 green, typecheck clean.*
