...
- [x] **3.2 Schema alignment** (marketing only): rework `marketing.publishEmailCampaign` onto `user_identities` email channel; `audience` trial/paid throws an actionable error (billing deferred until product, DECISIONS 2026-05-16). — *done 2026-05-16: PR #3 squash `290fce2`, CI green; TDD (2 tests), no `subscriptions`/`users.telegram_id` in marketing.*
- [x] **3.3 Prompt-table compliance**: support/qa currently bypass the `prompts` table (sentiment hardcoded + direct provider call; qa judge passes prompt text as prompt name → empty content). Refactor both to named shared prompts via `ask()`/`judge()` + seed migration `005_shared_agent_prompts.sql`. (Experiments has no AI calls — none needed.) — *done 2026-05-16: PR #4 squash `5b83c79`, CI green; migration 005 (project_id=NULL), TDD 5 tests, bot-core suite 13/13.*
